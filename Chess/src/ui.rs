...
const CHESS_BOARD_WIDTH: i32 = 521; // 棋盘实际宽度
const CHESS_BOARD_HEIGHT: i32 = 577; // 棋盘实际高度
...
trait BoardExt {
    ...
    fn move_to(&mut self, from: Position, to: Position);
...
impl BoardExt for Board {
    ...
    fn robot_move(&mut self) -> bool {
        ...
        if let Some(m) = best_move && m.is_valid() {
            self.do_move(&m);
            return true;
        }
        unreachable!();
